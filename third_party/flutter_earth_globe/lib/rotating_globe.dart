                  ...
                  child: LayoutBuilder(
                    builder:
                      ...
                      if (updatedCenter != center) {
                        // Was Future.delayed(Duration.zero, ...) — a
                        // real Timer/microtask that can take an
                        // unpredictable number of frames to resolve
                        // under load (e.g. during an active pinch-zoom,
                        // when this branch re-triggers on every frame
                        // the canvas needs to grow). addPostFrameCallback
                        // is the correct idiom for a setState triggered
                        // from within build(): it always resolves at the
                        // end of THIS frame, not an arbitrary later one,
                        // eliminating one source of the extra out-of-band
                        // rebuild cycle stacking up during zoom.
                        WidgetsBinding.instance.addPostFrameCallback((_) {
                          if (mounted) {
                            setState(() {
